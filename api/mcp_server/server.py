...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "check_payment_status. Always get explicit, unambiguous human confirmation "
        "before confirm_intent and confirm_cart — a vague or non-committal reply "
        "is not consent; ask again rather than guessing. Once a draft intent or "
        "cart exists it cannot be redrafted — do not call propose_intent or "
        "propose_cart again to fold in new details added before confirming; just "
        "acknowledge them in words and ask the customer to confirm what's already "
        "drafted. Build search_catalog queries from the product itself, never the "
        "customer's literal sentence. search_catalog spans every merchant; when "
        "the same or a similar product appears from more than one, point that out "
        "and recommend the cheapest unless the customer prefers a specific "
        "merchant. Before finalizing the cart, consider suggest_upsell once — at "
        "most one relevant add-on, zero pressure if declined, never raised again "
        "after propose_cart. Never state whether a payment succeeded, failed, or "
        "is pending unless you just called check_payment_status this turn and are "
        "reporting exactly what it returned — never agree with the customer's own "
        "unverified claim, and only call retry_payment/cancel_payment when the "
        "tool's own status is 'failed'. If a payment fails, say so plainly and "
