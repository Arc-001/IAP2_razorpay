...
<template>
  <div class="flex h-full flex-col">
    <div class="flex-1 space-y-3 overflow-y-auto p-4">
      <div v-if="conversation.displayLog.length === 0" class="flex h-full flex-col items-center justify-center gap-2 text-center">
        <span class="flex h-10 w-10 items-center justify-center rounded-full bg-indigo-50 text-lg">🛍️</span>
        <p class="max-w-xs text-sm text-slate-400">
          Tell the assistant what you're looking to buy — e.g.
          <span class="text-slate-500">"I want wireless earbuds, budget under 3000 rupees"</span>.
        </p>
      </div>
      ...
      <div v-if="conversation.isSending" class="flex justify-start">
        <div class="flex items-center gap-1 rounded-2xl border border-slate-200 bg-white px-4 py-3">
          <span class="h-1.5 w-1.5 animate-bounce rounded-full bg-slate-300 [animation-delay:-0.3s]"></span>
          <span class="h-1.5 w-1.5 animate-bounce rounded-full bg-slate-300 [animation-delay:-0.15s]"></span>
          <span class="h-1.5 w-1.5 animate-bounce rounded-full bg-slate-300"></span>
        </div>
      </div>
